...
INSERT INTO system_config (key, value, description, is_public) VALUES
    ...
    ('app_description', '"AI-Powered Sports Journalism Platform"', 'Application description', true),
    ('max_article_length', '10000', 'Maximum article length in characters', false),
    ('supported_sports', '["football", "basketball", "baseball", "soccer", "hockey"]', 'List of supported sports', true),
    ('article_generation_enabled', 'true', 'Whether AI article generation is enabled', false),
    ('comment_moderation_enabled', 'true', 'Whether comment moderation is enabled', false),
...
INSERT INTO agents.agent_configs (agent_type, name, description, configuration) VALUES
    ('data_collector', 'ESPN Data Collector', 'Collects game data from ESPN API', '{"api_endpoint": "https://api.espn.com", "rate_limit": 100, "timeout": 30}'),
    ('data_collector', 'Sports Reference Collector', 'Collects historical data from Sports Reference', '{"api_endpoint": "https://api.sports-reference.com", "rate_limit": 60, "timeout": 45}'),
    ('researcher', 'Background Researcher', 'Researches team and player backgrounds', '{"max_sources": 5, "research_depth": "medium", "fact_check_enabled": true}'),
    ('writer', 'Article Writer', 'Generates sports articles from game data', '{"tone": "professional", "max_length": 2000, "include_stats": true, "include_quotes": false}'),
    ('editor', 'Content Editor', 'Reviews and edits generated articles', '{"check_grammar": true, "check_facts": true, "improve_readability": true}'),
    ('fact_checker', 'Fact Checker', 'Verifies facts in articles', '{"sources_required": 2, "confidence_threshold": 0.8}'),
    ('seo_optimizer', 'SEO Optimizer', 'Optimizes articles for search engines', '{"keyword_density_target": 0.02, "meta_description_length": 160, "title_length_max": 60}')
...
CREATE OR REPLACE FUNCTION update_system_config(config_key TEXT, config_value JSONB)
RETURNS BOOLEAN AS $$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_agent_configs_agent_type ON agents.agent_configs(agent_type);
CREATE INDEX IF NOT EXISTS idx_agent_configs_is_active ON agents.agent_configs(is_active);
...
LEFT JOIN article_comments ac ON a.id = ac.article_id AND ac.is_approved = true
WHERE a.status = 'published'
GROUP BY a.id, a.title, a.sport, a.league, a.published_at
ORDER BY view_count DESC, comment_count DESC;

-- Create index on materialized view
CREATE UNIQUE INDEX IF NOT EXISTS idx_popular_articles_id ON popular_articles(id);
CREATE INDEX IF NOT EXISTS idx_popular_articles_sport ON popular_articles(sport);
CREATE INDEX IF NOT EXISTS idx_popular_articles_view_count ON popular_articles(view_count DESC);
