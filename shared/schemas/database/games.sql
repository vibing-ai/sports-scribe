...
CREATE TABLE games (
    ...
    CONSTRAINT games_status_check CHECK (status IN ('scheduled', 'pregame', 'in_progress', 'halftime', 'overtime', 'final', 'postponed', 'cancelled')),
...
CREATE TABLE players (
    ...
    CONSTRAINT players_status_check CHECK (status IN ('active', 'injured', 'suspended', 'inactive', 'retired', 'free_agent'))
);
...
CREATE TABLE game_moments (
    ...
    CONSTRAINT moments_type_check CHECK (type IN ('touchdown', 'field_goal', 'interception', 'fumble', 'sack', 'penalty', 'injury', 'substitution', 'timeout'))
);
...
CREATE TABLE player_injuries (
    ...
    CONSTRAINT injury_severity_check CHECK (severity IN ('minor', 'moderate', 'major', 'career_threatening')),
    CONSTRAINT injury_status_check CHECK (status IN ('active', 'recovering', 'questionable', 'cleared'))
);
...
CREATE POLICY "Public can read game stats" ON game_stats FOR SELECT USING (true);
CREATE POLICY "Public can read player game stats" ON player_game_stats FOR SELECT USING (true);
CREATE POLICY "Public can read game moments" ON game_moments FOR SELECT USING (true);
CREATE POLICY "Public can read team season stats" ON team_season_stats FOR SELECT USING (true);

-- Sensitive data like injuries require authentication
CREATE POLICY "Authenticated can read injuries" ON player_injuries FOR SELECT TO authenticated USING (true);

-- Only authenticated users can modify data
CREATE POLICY "Authenticated can insert venues" ON venues FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "Authenticated can update venues" ON venues FOR UPDATE TO authenticated USING (true);
