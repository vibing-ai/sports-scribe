...
CREATE TABLE user_profiles (
    ...
    CONSTRAINT user_profiles_role_check CHECK (role IN ('admin', 'editor', 'writer', 'viewer', 'api_user')),
...
CREATE TABLE user_notifications (
    ...
    CONSTRAINT notification_type_check CHECK (type IN ('article_published', 'game_alert', 'team_update', 'system_announcement', 'comment_reply'))
);
...
CREATE TABLE user_notification_preferences (
    ...
    CONSTRAINT frequency_check CHECK (frequency IN ('immediate', 'daily', 'weekly', 'never'))
);

-- Indexes for performance
CREATE INDEX idx_user_profiles_role ON user_profiles(role);
CREATE INDEX idx_user_profiles_is_active ON user_profiles(is_active);
CREATE INDEX idx_user_profiles_sports_interests ON user_profiles USING GIN(sports_interests);
CREATE INDEX idx_user_profiles_teams_following ON user_profiles USING GIN(teams_following);
...
CREATE POLICY "Public can view basic profile info" ON user_profiles
    FOR SELECT USING (
        ...
        id IN (SELECT DISTINCT author_id FROM articles WHERE status = 'published')
    );
